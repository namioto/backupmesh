...
- A Windows Source Agent package (`build-windows-source-package.ps1`) for backing up the same PC the Storage Agent runs on. `Install-BackupMeshSource.ps1` installs it entirely under `%LOCALAPPDATA%` with a per-user Scheduled Task, needing no administrator rights, and prompts for an Agent name and first Backup Set the same way the Linux installer now does. The tray's pairing dialog offers to pair a detected local installation automatically, so the endpoint/code/fingerprint never need to be typed by hand on the same PC. Verified with a real pairing, catalog sync, and Connections-list round trip against a live Storage Service on this machine; the Scheduled Task registration itself has not been exercised through a full install run in this session.
