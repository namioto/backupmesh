...
백업 가능한 저장장치가 연결되면 BackupMesh는 **설정** 탭에서 지정한 대기 시간이 지난 뒤 매핑된 백업을 대기열에 넣습니다. 기본 대기 시간은 30분입니다. **다음 백업**은 서비스가 계산한 실행 가능 시각이며 보장된 시작 시각은 아닙니다. 원격 에이전트가 오프라인이거나 사용 중이면 더 늦게 시작할 수 있습니다. **지금 백업**을 누르면 남은 대기 시간을 건너뜁니다. 이미 준비된 장치에 새 규칙을 추가한 경우 **지금 백업**으로 실행하세요. 이동식 저장장치 분리는 백업 작업이 끝난 뒤 Windows에서 수행합니다.
...
대상 장치를 연결하고 **다음 백업** 시각에 실행 가능 상태가 될 때까지 기다리거나, **지금 백업**을 눌러 남은 연결 대기 시간을 건너뜁니다. BackupMesh는 장치가 새로 준비 상태가 될 때 매핑된 백업을 대기열에 넣습니다. 이 동작은 장치 연결 기준 대기이며 정기 예약이 아닙니다. 트레이 앱에서 대기·실행 상태, 처리 파일과 바이트, 진행률, 결과, 최근 성공 시각을 확인할 수 있습니다. 실행 중인 작업을 취소하면 Source가 restic을 종료하고 `CANCELLED` 결과를 보고합니다.
