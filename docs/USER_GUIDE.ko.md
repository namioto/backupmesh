...
백업 가능한 저장장치가 연결되면 BackupMesh는 **설정** 탭에서 지정한 대기 시간이 지난 뒤 매핑된 백업을 대기열에 넣습니다. 기본 대기 시간은 1분입니다. **다음 백업**은 장치 준비 상태나 규칙의 자동 백업 간격을 보여주며 시작 시각을 보장하지는 않습니다. 원격 에이전트가 오프라인이거나 사용 중이면 더 늦게 시작할 수 있습니다. **지금 백업**을 누르면 남은 연결 대기 시간을 건너뜁니다. 이동식 저장장치 분리는 백업 작업이 끝난 뒤 Windows에서 수행합니다.
...
Storage에서 이 컴퓨터를 제거한 뒤 다시 연결하려면 `sudo backupmesh-setup`을 실행하세요. setup은 기본적으로 기존 연결을 유지하므로 질문에서 reset을 명시적으로 승인해야 합니다. reset은 백업 폴더와 repository 암호를 보존하고 새 신원을 만듭니다. 제거된 기존 신원은 계속 차단됩니다.
...
1. **백업할 데이터**에서 연결된 원격 컴퓨터의 Backup Set 또는 이 PC의 폴더를 선택합니다. 이어서 에이전트가 제공한 원본 경로 중 하나 이상을 선택합니다. 여러 경로를 선택해도 이 규칙의 저장 위치 하나로 백업됩니다.
2. **Where to store it**에서 연결된 드라이브를 선택하거나 **Choose folder…**를 누릅니다.
3. **Full destination path**에서 실제 전체 저장 경로를 확인하거나 수정합니다. BackupMesh는 선택한 폴더를 그대로 사용하며 자동으로 하위 폴더를 덧붙이지 않습니다.
4. **자동 백업 설정**에서 간격(5~1440분), PC 사용량이 높을 때 자동 백업 미루기, 원격 규칙별 업로드 속도 제한을 설정합니다. 속도 제한을 비우면 에이전트 설정을 따르고 `0`이면 이 규칙에는 제한을 적용하지 않습니다.
5. **백업 추가**를 누릅니다. 기존 행을 더블클릭하면 규칙을 수정할 수 있습니다. 원본·대상 장치·대상 폴더가 모두 같은 규칙은 중복 저장되지 않습니다.
...
대상 장치를 연결하고 기본 1분의 연결 대기 시간이 지나기를 기다리거나 **지금 백업**으로 남은 시간을 건너뜁니다. BackupMesh는 장치가 준비되면 백업을 대기열에 넣고, 연결 상태가 유지되는 동안 규칙에서 정한 간격마다 다시 실행합니다. 파일이 바뀌지 않아도 restic이 원본을 확인하며 변경된 데이터만 저장합니다. PC가 바쁠 때 미루기 옵션은 자동 백업에만 적용되고 **지금 백업**에는 적용되지 않습니다. 트레이 앱에서 대기·실행 상태, 처리 파일과 바이트, 진행률, 결과, 최근 성공 시각을 확인할 수 있습니다. 실행 중인 작업을 취소하면 Source가 restic을 종료하고 `CANCELLED` 결과를 보고합니다.
