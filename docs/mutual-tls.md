...
On each Source Agent, use an `https` `controlEndpoint` and set all three absolute paths: `tlsCaFile`, `tlsCertificateFile`, and `tlsKeyFile`. Issue the client certificate with the Source Agent UUID as its Common Name (CN); it must exactly match `agent.id`. The CA validates the Storage Agent and the certificate/key identify that Source. BackupMesh permits TLS 1.3 only on this endpoint. The bearer token remains available as a migration fallback and should be removed after every Source has its own certificate.
