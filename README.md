...
Current version: **0.3.1** — the Storage app supports Korean and English, displays project information and GitHub links, and offers optional settings cleanup during uninstall. See the [changelog](CHANGELOG.md).
...
To pair a Remote Agent, choose **Pair a Remote Agent** on the **Remote Agents** tab. Enter the displayed Storage address, ten-minute one-time code, and certificate SHA-256 fingerprint with `backupmesh-agent pair`. The Remote Agent pins the Storage certificate before transmitting the code and installs its identity-bound token, client certificate, private key, and Storage trust material with owner-only permissions. New pairing never places a private key in a transfer bundle or modifies the operating-system trust store.
