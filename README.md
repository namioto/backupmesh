...
Current release: **0.2.1** — backup setup now focuses on two choices: what to back up and the full destination path. Storage identity is managed internally, the rule editor shows complete paths, and transient tray notifications no longer remain pinned. See the [changelog](CHANGELOG.md).
