...
& (Join-Path $PSScriptRoot 'build-windows-source-package.ps1') -Runtime $Runtime
if ($LASTEXITCODE -ne 0) { throw 'Windows Source Agent package build failed.' }
...
$packageRoot = Join-Path $repoRoot "artifacts\BackupMesh-Source-$Runtime"
$outputRoot = Join-Path $repoRoot 'artifacts\installer'
New-Item -ItemType Directory -Path $outputRoot -Force | Out-Null
$definition = Join-Path $repoRoot 'packaging\windows\BackupMeshSource.iss'
& $compiler '/Qp' "/DAppVersion=$version" "/DSourcePackage=$packageRoot" "/DOutputDirectory=$outputRoot" $definition
if ($LASTEXITCODE -ne 0) { throw 'Inno Setup compilation failed.' }

$installer = Join-Path $outputRoot "BackupMesh-Source-$version-win-x64-Setup.exe"
...
Write-Host "Windows Source Agent installer: $installer"
