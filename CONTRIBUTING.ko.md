...
회귀 발생 시 피해와 다른 기능에 미치는 영향을 스펙 변경에 따른 유지 비용과 비교해 테스트 필요성을 판단합니다. 단순 표시·조회 기능에는 기본적으로 전용 테스트를 추가하지 않습니다. 핵심 상태 전이, 신원, 데이터 무결성, 기능 간 부작용을 보호하는 테스트는 유지하고, 가치가 낮은 검증을 남기려고 실행 분류를 추가하지 않습니다.
