...
현재 릴리스: **0.2.1** — 백업 설정을 무엇을 백업할지와 전체 저장 경로를 고르는 흐름으로 단순화했습니다. 저장장치 식별은 내부에서 관리하고, 규칙 편집기에는 실제 경로를 표시하며, 트레이 알림은 계속 고정되지 않고 자동으로 닫힙니다. 자세한 내용은 [변경 기록](CHANGELOG.md)을 확인하세요.
...
일반 Windows 사용자는 `pwsh -NoProfile -File scripts/build-windows-installer.ps1`로 `BackupMesh-Storage-0.2.1-win-x64-Setup.exe`를 만든 뒤 설치 프로그램을 실행하면 됩니다. 서비스, 트레이 앱, 방화벽 규칙, 번들 도구, 자동 시작, 제거 프로그램을 한 번에 설치하고 완료 후 BackupMesh를 실행합니다. 설치 프로그램 빌드에는 [Inno Setup 6](https://jrsoftware.org/isinfo.php)이 필요합니다.
