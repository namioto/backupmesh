...
현재 버전: **0.3.7** — Ubuntu 원격 에이전트 설정 마법사에서 JSON 편집이나 요청 ID 복사 없이 백업 폴더를 추가하고 근처 연결을 승인할 수 있습니다. 자세한 내용은 [변경 기록](CHANGELOG.md)을 확인하세요.
...
양쪽 에이전트가 0.3.6 이상이면 Windows에 로그인할 때 설치된 감시 프로그램이 자동으로 시작됩니다. **원격 에이전트** 탭의 **근처 컴퓨터**에서 선택하고 **연결 요청**을 누르세요. 양쪽 컴퓨터에 같은 6자리 비교 번호가 표시되는지 확인한 뒤 원격 컴퓨터에서 승인합니다. 근처 컴퓨터 이름은 연결이 완료되기 전까지 확인되지 않은 정보입니다. 탐색이 차단되거나 서로 다른 서브넷을 사용할 때는 기존 연결 초대를 계속 사용할 수 있습니다.
